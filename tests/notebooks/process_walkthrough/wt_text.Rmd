...
cat("\n\n")
inspect(x = processing_text, m = 580, span = 25)
```
...
# replacement_patterns <- HypothesisReader::encoding_replacement
# 
# length(encoding_errors)
# 
# length(replacement_patterns)
# 
...
#   processing_text <- stringr::str_replace(
...
# length(processing_text)
# cat("\n\n")
# inspect(x = processing_text, m = 580, span = 20)
```

### Function
```{r}
processing_text <- HypothesisReader::fix_encoding_errors(processing_text)
...
length(processing_text)
cat("\n\n")
inspect(x = processing_text, m = 360)
```
...
# Check if Section Header is present anywhere in document
if (any(logical_section)){
  
  # Determine all instances of reference section
  ref_indexes <- which(logical_section == TRUE)
  
  # Determine document index where after reference section is dropped
  document_length <- length(processing_text)
  print(document_length)
  ref_cut_off_percent <- 0.8
  ref_cut_off_index <- round(document_length * ref_cut_off_percent, 0)
  print(ref_cut_off_index)
  print(max(ref_indexes))
  ...
  if (max(ref_indexes) >= ref_cut_off_index) {
    ...
    print("Reference section dropped")
    print(index)
...
length(processing_text)
cat("\n\n")
inspect(x = processing_text, m = 240)
```
...
length(processing_text)
cat("\n\n")
inspect(x = processing_text, m = 225)
```
...
length(processing_text)
cat("\n\n")
inspect(x = processing_text, m = 235)
```
...
length(processing_text)
cat("\n\n")
inspect(x = processing_text, m = 150)
```
...
length(processing_text)
cat("\n\n")
inspect(x = processing_text, m = 60, span = 10)
```
...
length(processing_text)
cat("\n\n")
inspect(x = processing_text, m = 80, span = 10)
```
...
length(processing_text)
cat("\n\n")
inspect(x = processing_text, m = 75, span = 10)
```
...
inspect(x = processed_text, m = 75, span = 10)
```
